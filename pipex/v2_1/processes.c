...
/*   Updated: 2023/05/02 20:19:03 by amak             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*	Free variables from struct pipex used in the process functions: array of
	command flags, path and command */
...
/*	First child process:
	- checks if first command value is NULL, if not will execute:
	- redirects the file descriptors: STDIN to infile and STDOUT to pipe[1] - 
	  (write), closes pipe[0] - (read)
	- splits command and flags to an array to use in "execve" function
	- if "execve" executes it terminates the process, else (because the command
	  path is NULL) it prompts "command not found error" and frees all the
	  variables of struct pipex that was used and exit the process */
